...
// DataType: a variable of type `DataType` is really stored
// inside this class,usually the same as `Pointer`.

// T: the basic of return type of deferenced iterators

template <class T,class DataType,class Tag = std::forward_iterator_tag,
          class ValueType = T,class Reference = T&,class Pointer = T*>
class iterator_base{
    public:
        typedef iterator_base<T,DataType,Tag,ValueType,Reference,Pointer> self_type; // not required for STL-like iterator
        typedef DataType  data_type; // not required for STL-like iterator
        typedef ValueType value_type;
        typedef Reference reference;
        typedef Pointer   pointer;
        typedef long int  difference_type;
        typedef Tag       iterator_category;
        explicit iterator_base(const data_type _ptr);
...
template <class T,class DataType,class Tag,class ValueType,class Reference,class Pointer> iterator_base<T,DataType,Tag,ValueType,Reference,Pointer>::iterator_base(const data_type _ptr):ptr(_ptr) {}
...
template <class T,class DataType,class Tag,class ValueType,class Reference,class Pointer> typename iterator_base<T,DataType,Tag,ValueType,Reference,Pointer>::data_type  iterator_base<T,DataType,Tag,ValueType,Reference,Pointer>::get() const {return ptr;}
