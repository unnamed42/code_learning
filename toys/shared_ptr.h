...
template <> struct deleter<std::nullptr_t>{
    constexpr deleter() noexcept =default;
...
class shared_ptr{
    ...
    public:
        explicit shared_ptr(T *ptr):deleter(Deleter()) { _data=new data; _data->ptr=ptr; _data->count=1; }
        
        constexpr explicit shared_ptr(std::nullptr_t):_data(nullptr),deleter(Deleter()) {}
